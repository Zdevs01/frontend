...
const imageMap = {
  ...
  'cafard.jpg': require('@/assets/images/th1/cafard.jpg'),
...
const rawData = [
  { label: "Les abeilles", correct: 'abeilles.png', audio: require('@/assets/sounds/bassa/th1/abeilles.wav'), images: ['abeilles.png', 'sauterelle.png', 'cafard.jpg', 'épervier.jpg'] },
  { label: "La sauterelle", correct: 'sauterelle.png', audio: require('@/assets/sounds/bassa/th1/sauterelle.wav'), images: ['sauterelle.png', 'lion.jpg', 'cafard.jpg', 'épervier.jpg'] },
  { label: "🪳 Le cafard", correct: 'cafard.jpg', audio: require('@/assets/sounds/bassa/th1/cafard.wav'), images: ['épervier.jpg', 'cafard.jpg', 'abeilles.png', 'poisson.jpg'] },
  ...
  { label: "Le Lion", correct: 'lion.jpg', audio: require('@/assets/sounds/bassa/th1/lion.wav'), images: ['épervier.jpg', 'lion.jpg', 'cafard.jpg', 'abeilles.png'] },
  { label: "epervier", correct: 'épervier.jpg', audio: require('@/assets/sounds/bassa/th1/epervier.wav'), images: ['abeilles.png', 'sauterelle.png', 'cafard.jpg', 'épervier.jpg'] }
];
...
export default function ex3_t1_3() {
  ...
  useEffect(() => {
    ...
    const loadUser = async () => {
      ...
      if (userData) {
        ...
        setUserId(user.id);
        const res = await fetch(`${API_URL}/api/user/stats/${user.id}`);
  ...
  const handleSelection = async (index) => {
    ...
    if (index === data[current].correctIndex) {
      ...
      const res = await fetch(`${API_URL}/api/user/remove-cauri/${userId}`);
      const json = await res.json();
    ...
    setTimeout(async () => {
      ...
      if (index === data[current].correctIndex) {
        if (current + 1 < data.length) {
          ...
          setXpEarned(score);
          setShowFinal(true);
          await fetch(`${API_URL}/api/theme2/advance`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId, next: 7 })
          });
          await fetch(`${API_URL}/api/user/update-xp-time`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
